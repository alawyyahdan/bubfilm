...
interface TVDetailClientProps {
  ...
  initialSeason: { episodes: Episode[] };
}

export default function TVDetailClient({ show, initialSeason }: TVDetailClientProps) {
  if (!show) return null;
  const tv = show;
  const [currentSeason, setCurrentSeason] = useState(1);
  const [currentEpisode, setCurrentEpisode] = useState(1);
  const [episodes, setEpisodes] = useState<Episode[]>(initialSeason?.episodes || []);
  const [loadingEps, setLoadingEps] = useState(false);
  const [showPlayer, setShowPlayer] = useState(false);
  const [showTrailer, setShowTrailer] = useState(false);

  const trailer = (tv.videos as { results?: any[] })?.results?.find(
  ...
  const [inList, setInList] = useState(() => {
    ...
    return list.some((i: { id: number }) => i.id === show.id);
  });

  const seasons = ((show.seasons as Season[]) || []).filter((s) => s.season_number > 0);
  const genres = (show.genres as { id: number; name: string }[]) || [];
  const similar = (show.similar as { results: Record<string, unknown>[] }) || { results: [] };
  const credits = (show.credits as { cast: { id: number; name: string; profile_path: string | null; character: string }[] }) || { cast: [] };
  const rating = ((show.vote_average as number) || 0).toFixed(1);
  const year = ((show.first_air_date as string) || "").slice(0, 4);
  ...
  const handleSeasonChange = async (s: number) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_TMDB_BASE_URL}/tv/${show.id}/season/${s}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}`
      );
  ...
  const toggleList = () => {
    const list = JSON.parse(localStorage.getItem("mylist") || "[]");
    if (inList) {
      localStorage.setItem("mylist", JSON.stringify(list.filter((i: { id: number }) => i.id !== show.id)));
    } else {
      list.push({ id: show.id, type: "tv", title: show.name, poster_path: show.poster_path });
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="relative h-[55vh] sm:h-[60vh] overflow-hidden">
        {show.backdrop_path && (
          <Image
            src={tmdbImg(tv.backdrop_path as string, "original")}
            alt={tv.name as string}
        ...
        {/* Back */}
        <Link href="/tv" className="absolute top-20 left-6 flex items-center gap-2 text-gray-300 hover:text-white text-sm transition-colors z-20">
          <ChevronLeft size={18} /> TV Shows
        </Link>
      </div>
      ...
      <div className="max-w-screen-2xl mx-auto px-6 sm:px-8 -mt-32 relative z-10 pb-16">
        ...
        <div className="flex flex-col sm:flex-row gap-8 mb-10">
          ...
          <div className="flex-shrink-0 w-44 sm:w-52 lg:w-60 mx-auto sm:mx-0">
            <div className="relative aspect-poster rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10">
              {Boolean(show.poster_path) && (
                <Image src={tmdbImg(show.poster_path as string, "w500")} alt={show.name as string} fill className="object-cover" sizes="240px" />
              )}
            </div>
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4 text-sm text-gray-400">
              ...
              <span className="flex items-center gap-1"><Calendar size={13} />{year}</span>
              <span className="text-xs border border-gray-600 px-1.5 py-0.5 rounded">
                {show.number_of_seasons as number} Season{(show.number_of_seasons as number) > 1 ? "s" : ""}
              </span>
            ...
            <p className="text-gray-300 text-sm sm:text-base leading-relaxed mb-6 max-w-2xl">{show.overview as string}</p>
            <div className="flex flex-wrap gap-3">
              ...
              >
                ...
                {showPlayer ? "Hide Player" : `Play S${currentSeason}E${currentEpisode}`}
              </button>
              <button
                onClick={toggleList}
          ...
          <div className="mb-12">
            {loadingEps ? (
              ...
              <EpisodeList
                ...
                onEpisodeSelect={(ep) => { setCurrentEpisode(ep); setShowPlayer(true); window.scrollTo({ top: 0, behavior: "smooth" }); }}
              />
            )}
          </div>
        )}
        ...
        {similar.results.length > 0 && (
          <ContentRow title="Similar Shows" items={similar.results as unknown as import("@/lib/tmdb").TMDBMedia[]} type="tv" />
        )}
      </div>
    </div>
  );
