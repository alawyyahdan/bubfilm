...
export async function generateMetadata({ params }: { params: Promise<{ sport: string }> }): Promise<Metadata> {
  ...
  const displayName = sport.replace(/-/g, " ").replace(/\b\w/g, c => c.toUpperCase());
  return {
    title: `${sportEmoji[sport] || "🏆"} ${displayName} - Live & Upcoming`,
...
export default async function SportCategoryPage({ params }: { params: Promise<{ sport: string }> }) {
  ...
  const displayName = validSport?.displayName
    ...
    <div className="min-h-screen bg-zinc-950 pt-24 pb-16">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="px-4 sm:px-6 lg:px-10 mb-8 border-b border-white/5 pb-6">
          ...
          <p className="text-zinc-400 mt-2 text-sm">Live & upcoming {displayName} matches.</p>
        </div>
